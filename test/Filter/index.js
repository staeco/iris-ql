...
describe('Filter', () => {
  ...
  it('should work with JSON subkeys when schema provided', async () => {
    ...
    should.exist(query.toJSON())
    should.exist(query.input)
    should(where({ value: query.value(), model: datum })).eql(
      `(parse_iso("datum"."data"#>>'{startedAt}') <= now() AND "datum"."data"#>>'{startedAt}' IS NOT NULL)`
    )
  })
  it('should work with array JSON subkeys when schema provided', async () => {
    ...
    should.exist(query.input)
    should(where({ value: query.value(), model: datum })).eql(
      `fix_jsonb_array("datum"."data"#>>'{officers}') @> ARRAY['W301']`
    )
  })
  it('should fail with invalid function usage', async () => {
    try {
      new Filter(
        {
          function: 'gte',
          arguments: [{ field: 'data.officers' }, 'abc']
        },
        {
          model: datum,
          subSchemas: {
      ...
      should(err.fields).eql([
        {
          ...
          message:
            'Argument "Value A" for "Greater Than or Equal" must be of type: number, date - instead got array'
        }
      ])
      return
    }
