...
describe('Query#options#before', () => {
  ...
  it('should work for valid before values', async () => {
    ...
    should.exist(new Query({ before: new Date().toISOString() }, { model: user }))
  })
  ...
  it('should execute with before', async () => {
    const query = new Query({ before: new Date().toISOString(), limit: 1 }, { model: user })
  ...
  it('should execute with before and no results', async () => {
    const query = new Query({ before: new Date('1/1/1975').toISOString() }, { model: user })
    const res = await query.execute()
    should.exist(res.count)
    should.exist(res.rows)
