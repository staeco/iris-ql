...
describe('Query#options#filters', () => {
  ...
  it('should work for valid filters values', async () => {
    should.exist(new Query({ filters: {} }, user))
    should.exist(new Query({ filters: { name: { $ne: null } } }, user))
  })
  it('should return 400 on bad filters', async () => {
    should.throws(() => new Query({ filters: 'blahblah' }, user))
  })
  it('should execute with filters', async () => {
    const query = new Query({ filters: { name: { $eq: 'Yo Yo 1' } } }, user)
