...
describe('Ordering', () => {
  ...
  it('should blow up on invalid parsing options', async () => {
    ...
    should.throws(() =>
      new Ordering({ value: 'id', direction: NaN }, { model: user }).value()
    )
    should.throws(() => new Ordering({ value: 'id' }, { model: user }).value())
    should.throws(() =>
      new Ordering({ direction: 'asc' }, { model: user }).value()
    )
    should.throws(() =>
      new Ordering(
        { value: { function: 'wat' }, direction: 'asc' },
        { model: user }
      ).value()
    )
  })
  it('should return an ordering query when valid', async () => {
    const t = new Ordering(
...
describe('Ordering#parse', () => {
  ...
  it('should blow up on invalid parsing options', async () => {
    ...
    should.throws(() =>
      parse({ value: 'id', direction: NaN }, { model: user }).value()
    )
