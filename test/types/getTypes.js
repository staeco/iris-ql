...
describe('types#getTypes', () => {
  ...
  it('should work on plain values', () => {
    ...
    should(fn(new Date().toISOString())).eql([
      { type: 'text' },
      { type: 'date' }
    ])
    should(fn({ type: 'Point', coordinates: [1, 1] })).eql([
      { type: 'object' },
      { type: 'point' }
    ])
  })
  ...
  it('should work on functions that bubble up types', () => {
    should(
      fn({ function: 'max', arguments: [{ field: 'data.startedAt' }] }, opt)
    ).eql([{ type: 'date' }])
    should(
      fn({ function: 'max', arguments: [{ field: 'data.cost' }] }, opt)
    ).eql([
      {
        type: 'number',
        measurement: {
  ...
  it('should work on functions that bubble up measurements', () => {
    should(
      fn({ function: 'add', arguments: [{ field: 'data.cost' }, 1] }, opt)
    ).eql([
      {
        type: 'number',
        measurement: {
  ...
  it('should work on plain fields', () => {
    should(fn({ field: 'data.startedAt' }, opt)).eql([
