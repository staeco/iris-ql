...
export default class Query {
  ...
  update = (fn) => {
    ...
    if (!newInstanceValue || typeof newInstanceValue !== 'object')
    ...
    if (!newCollectionValue || typeof newCollectionValue !== 'object')
      throw new Error('Invalid update function! Must return an object.')
  ...
  value = ({ instanceQuery = true } = {}) =>
    instanceQuery ? this._parsed : this._parsedCollection
  toJSON = () => this.input
  getOutputSchema = () => {
    let fieldLimit =
      this.options.fieldLimit || getModelFieldLimit(this.options.model)
    if (this.input.exclusions) {
      fieldLimit = fieldLimit.filter(
        (i) => !this.input.exclusions.includes(i.field)
      )
    }
