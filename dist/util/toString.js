  ...
  if (analytics) {
    ...
    if (joinStr) {
      if (value.attributes) {
        ...
        const selectSubstring = basic.match(new RegExp('SELECT (.*) FROM'))[1]; // inject attributes into union statements, replacing '*'
    ...
    let statementEnd = ';'; // if limit, separate primary query out to inject limit statement at the end
...
const unionAll = ({
  ...
  const qg = getQueryGenerator(model); // select statement in format: SELECT alias AS _alias, SELECT * FROM table

  const unionStr = qg.selectQuery(model.getTableName(), where, model).replace('SELECT', `SELECT '${alias}' AS _alias,`).slice(0, -1); // return with where statement if applicable
