...
function _ref3(acc, [k, v]) {
  return (0, _fixJSONFilters.hydrate)(acc, { ...v,
...
var _default = (obj, opt) => {
  ...
  const transformValues = (v, parent = '', idx) => {
    ...
    if ((0, _isQueryValue.default)(v)) {
      return new _QueryValue.default(v, { ...opt,
        context: ctx,
  ...
  const transformed = transformValues(obj); // turn where object into string with fields hydrated

  if (!error.isEmpty()) throw error;
  const out = (0, _fixJSONFilters.hydrate)((0, _fixJSONFilters.unwrap)(transformed, opt), opt);
  if (!opt.joins) return out; // run through all of our joins and fix those up too
